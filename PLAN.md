...
- [x] Task 5.2: Crawl 2-3 website thật (agency/marketing SaaS). Viết script `crawl_websites.py` chạy 1 lần để setup.
- [x] Task 5.3: Cập nhật `README.md` + `EXPLAINATION.md` mô tả kiến trúc mới.

## Phase 6: Gradio Interface [x]

- [x] Task 6.1: Viết `SPEC.md` cho giao diện Gradio thay thế Telegram.
- [x] Task 6.2: Refactor `src/chat.py`:
  - Thêm `SYSTEM_PROMPT_GENERAL` (chưa crawl → general LLM)
  - Rename `SYSTEM_PROMPT` → `SYSTEM_PROMPT_RAG` (đã crawl → RAG-only)
  - Thêm `chat_general_stream()` (LLM streaming không RAG)
  - Thêm `chat_rag_stream()` (RAG + LLM streaming)
  - Refactor `chat_stream()` nhận param `kb_ready: bool`
- [x] Task 6.3: Viết `app_gradio.py` (~200 dòng):
  - gr.Blocks layout: chat trái, panel phải
  - Model selector: 4 button (gpt-4o-mini, gpt-4o, o4-mini, gpt-4o-realtime)
  - URL input + Fetch button + X button + Clear KB
  - Warning message tự động sau crawl / clear
  - Streaming với gr.ChatInterface-style partial updates
- [x] Task 6.4: Thêm `gradio>=5.0` vào `requirements.txt`.
- [x] Task 6.5: Cập nhật `README.md` + `EXPLAINATION.md` mô tả Gradio interface.
- [x] Task 6.6: Tests cho `chat_general_stream` + `chat_rag_stream`.
